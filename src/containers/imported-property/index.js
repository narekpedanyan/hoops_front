...
import {setImportedProperty} from "../../store/actions/import-actions";
...
class ImportedProperty extends Component {
    ...
    onChange(e) {
        ...
        const {propertyItem: {name}} = this.props;
        const payload = {name, value};
        this.props.dispatch(setImportedProperty(payload));
    }

    render() {
        const {propertyItem: {name}, propertyOptions} = this.props;
        const storeData = this.props.importCatalogFiles.properties.find(item => item.name === name);
        let value = '';
        if (storeData) {
            value = storeData.value;
        }
        return (
            <PropertyItem name={name} required={this.state.required} onChange={this.onChange} value={value} options={propertyOptions} />
        )
    }
}
