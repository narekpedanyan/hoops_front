...
class SelectDropdown extends Component{
    ...
    hendleOptionValue(value){
        this.props.dispatch(SetSelectedVendor(value));
        this.setState({
            dropOpen: false
        })
    }
    ...
    render(){
        ...
        return(
            <div className='select-drop'>
                <div className='value' onClick={() => this.toggleDropdown()}>{selectVendor.selected === null ? 'Select drop' : selectVendor.selected}</div>
                {
                    this.state.dropOpen && (
                        <div className='dropdown'>
                            ...
                            <div className='options'>
                                <div className='shared-scroll-view'>
                                    ...
                                    {
                                        selectVendor.vendorList.data && selectVendor.vendorList.data.map((item) => (<div className='option' key={item.id} onClick={() => this.hendleOptionValue(item["vendor_name"])}>{item["vendor_name"]}</div>))
                                    }
                                </div>
                            </div>
                        </div>
